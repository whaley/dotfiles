...
def find_all_obsidian_window_ids() -> List[int]:
  ...
  return [line[0].strip() for line in split_lines if line and line[1].strip() == "Obsidian"]
...
if __name__ == "__main__":
  ...
  if obsidian_window_ids:
    ...
    for window_id in obsidian_window_ids:
      subprocess.run([AEROSPACE, "move-node-to-workspace", "--window-id", window_id, active_workspace_name])
  else:
